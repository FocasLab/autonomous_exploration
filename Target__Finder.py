...
with open(filename, "r") as ip:
    inp = [line.strip() for line in ip] # Split the text file line by line and store in an array
x = 0 # variable to store the grid elements
for i in range(len(inp)):
    inp[i] = inp[i].split() # Split each line
    if inp[i][0] == 'data:':
        d = []
        x = float((inp[i][1].split('[')[1]).split(',')[0])
        if x>0:d.append(x/100) # Replacing all the 100s(obtacles found) with 1
        else: d.append(x)
        for j in range(2, len(inp[i])-1):
            x = float(inp[i][j].split(',')[0])
            if x>0: d.append(x/100)
            else: d.append(x)
        x = float(inp[i][len(inp[i])-1].split(']')[0])
        if x>0: d.append(x/100)
        else: d.append(x)
        maps.append(d)
# Chaning the obtained grid to a 2D matrix
for i in range(len(maps)):
    maps[i] = np.array(maps[i]).reshape(384, 384)
